...
# plot 
ggplot(wimean, mapping = aes(x=end_date, y=pct_03da, color=answer))+
  geom_jitter() +
...
|-------------|-------|-----------|------------------------|
| Jul 10,2024 | 41%   | 43.3%     | **Head-to-Head**       |
| Jul 10,2024 | 42.3% | **42.7%** | with **Third-Parties** |
...
# plot 
ggplot(pamean, mapping = aes(x=end_date, y=pct_03da, color=answer))+
  geom_jitter()+
    geom_smooth(method = 'loess', formula = 'y~x',span=0.5, se=FALSE, lwd=1)+
  scale_colour_manual(values = c('#0099ff','#ff6666')) +
...
| End Date    | Biden | Trump     | Ballot                 |
|-------------|-------|-----------|------------------------|
| Jul 11,2024 | 40.7% | **45%**   | **Head-to-Head**       |
| Jul 11,2024 | 42%   | **46.6%** | with **Third-Parties** |
...
# plot 
ggplot(mimean, mapping = aes(x=end_date, y=pct_03da, color=answer))+
  geom_jitter() +
    geom_smooth(method = 'loess', formula = 'y~x',span=0.5, se=FALSE, lwd=1)+
  scale_colour_manual(values = c('#0099ff','#ff6666')) +
  scale_y_continuous(limits = c(min(30),max(50)))+
...
| End Date    | Biden | Trump     | Ballot                 |
|-------------|-------|-----------|------------------------|
| Jul 10,2024 | 39.7% | **41.3%** | **Head-to-Head**       |
| Jul 10,2024 | 41%   | **42%**   | with **Third-Parties** |
